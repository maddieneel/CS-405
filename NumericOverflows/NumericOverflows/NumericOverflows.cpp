...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    //TODO - referenced https://www.geeksforgeeks.org/check-for-integer-overflow/amp/
     if ((start >= 0 && increment >= 0 && (start > std::numeric_limits<T>::max() - increment)) || (start < 0 && increment < 0 && (start < std::numeric_limits<T>::max() - increment))) {
        //std::cout << "Addition Failed ";
        return false; //For test_overflow to know addition failed
    }
    else {
        result = start + (increment * steps);
        return result; //Return the correct value when no overflow happened
    }
}
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    //TODO
    //if ((start >= 0 && decrement >= 0 && result > std::numeric_limits<T>::min()) || (start < 0 && decrement < 0 && result > std::numeric_limits<T>::min())) { //Detect when an underflow will happen
    if ((start <= 0 && decrement <= 0 && (start < std::numeric_limits<T>::min() - decrement))) { //Det 
    //std::cout << "Subtraction Failed ";
        return false; //For test_underflow to know subtraction failed
    }
    else {
        result = start - (decrement * steps);
        return result; //Return the correct value when no underflow happened
    }
}
...
void test_overflow()
{
    ...
    //TODO
    if (result == false) { //Detect when an add_numbers failed
        std::cout << "Addition Failed" << std::endl; //Inform the user the addition failed
    }
    else if (result != false) {
        std::cout << +result << std::endl; //successful output
    }
    else {
        std::cout << "Error" << std::endl; //safety - double check error
    }
    ...
    //TODO
    if (result == false) { //Detect when an add_numbers failed
        std::cout << result << "Addition Failed" << std::endl; //Inform the user the addition failed
    }
    else if ((result != false) && (result > std::numeric_limits<T>::max())){ //if no overflow occured
        std::cout << +result << std::endl; //successful result displays the same result as before you changed the method
    }
    else if ((result != false) && (result < std::numeric_limits<T>::max())) { //if overflow occured
        result = start + (increment * steps); //successful result displays the same result as before you changed the method
        std::cout << +result << " Overflow Caught" << std::endl; //announce to user
    }
    else {
        std::cout << "Error" << std::endl; //safety - double check error
    }
...
void test_underflow()
{
    ...
    //TODO
    if (result == false) { //Detect when an subtract_numbers failed
        std::cout << "Subtraction Failed" << std::endl; //Inform the user the subtraction failed
    }
    else if (result != false){
        std::cout << +result << std::endl; //successful output
    }
    ...
    else if ((result != false) && (result > std::numeric_limits<T>::min())) { //if no underflow occured
        std::cout << +result << std::endl; //successful result displays the same result as before you changed the method
    }
    else if ((result != false) && (result < std::numeric_limits<T>::min())) { //if underflow occured
        result = start - (decrement * steps); //successful result displays the same result as before you changed the method
        std::cout << +result << "Underflow Caught" << std::endl;
    }
    else{
        std::cout << "Error" << std::endl; //safety - double check error
    }
