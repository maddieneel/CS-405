...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    if (sql.find(" or ") != std::string::npos) { //flag 'or' - more generic & in each injection phrase; lines 116-131
        std::cout << "SQL Injection" << std::endl; //notify user if SQL injection is suspected
