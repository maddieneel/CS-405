...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
    ...
    { // TODO: student need to change the next line from output[i] = source[i]
      // transform each character based on an xor of the key modded constrained to key length using a mod
        //reference used: https://www.programmingalgorithms.com/algorithm/xor-encryption/cpp/
...
std::string read_file(const std::string& filename)
{
    ...
    //reference used: https://www.delftstack.com/howto/cpp/read-file-into-string-cpp/
    
    std::string path = filename; //"inputdatafile.txt"
    std::ifstream file(path); //input stream; reading from data files
    
    if (!file.is_open()) { //if the file is not open
        std::cerr << "Could not open file: " << path << std::endl; //inform user
        exit(EXIT_FAILURE);
    }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    //time referenced from: https://stackoverflow.com/questions/5438482/getting-the-current-time-as-a-yyyy-mm-dd-hh-mm-ss-string
    std::time_t rawtime;
    std::tm* timeinfo;
    char buffer[80];
    std::time(&rawtime);
